    ...
    <Chrome>
      <div className="mx-auto h-full max-w-5xl overflow-y-auto px-4 py-6">
        ...
        {left && right ? (
          ...
          <p className="mt-8 text-sm text-muted">{t.comparePick}</p>
        )}
      </div>
    </Chrome>
  );
...
function CompareBody({
  ...
  const sameCurrency = CITY_BY_ID[left.cityId].currency === CITY_BY_ID[right.cityId].currency;
  const sameCity = left.cityId === right.cityId;
    ...
    <div className="mt-6 space-y-6">
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <Big
          ...
          value={sameCurrency ? `${gap >= 0 ? "+" : "−"}${formatOutcome(Math.abs(gap), "hhIncome", right.cityId)}` : (lang === "zh" ? "币种不同" : "Different currencies")}
      ...
      <p className="text-sm text-muted">{t.exposureHint}</p>
      <div className="grid gap-3 sm:grid-cols-3">
        <Stat
          label={t.moveBirth}
          value={sameCity ? formatOutcome(exposedIncome(lm, rm, 0), "hhIncome", right.cityId, true) : (lang === "zh" ? "仅支持同城模拟" : "Same-city only")}
        />
        <Stat
          label={t.move10}
          value={sameCity ? formatOutcome(exposedIncome(lm, rm, 10), "hhIncome", right.cityId, true) : (lang === "zh" ? "仅支持同城模拟" : "Same-city only")}
        />
        <Stat
          label={lang === "zh" ? "环境评分" : "Context score"}
          value={`${Math.round(neighborhoodQuality(left) * 100)} → ${Math.round(neighborhoodQuality(right) * 100)}`}
        />
      </div>

      <div>
        <div className="mb-2 text-sm tracking-wide text-muted uppercase">{t.covariates}</div>
        <div className="space-y-2">
          {COVARIATE_KEYS.map((k) => (
            <div key={k} className="grid grid-cols-[8rem_1fr_auto] items-center gap-2 text-sm md:grid-cols-[12rem_1fr_auto]">
