...
export function DistrictDetail({ onClose }: { onClose?: () => void }) {
  ...
  if (!d) {
    ...
    return (
      <div className="flex h-full flex-col gap-4 p-5">
        <div>
          <div className="text-sm text-muted">{t.click}</div>
          <h2 className="font-display text-2xl text-paper">
            {lang === "zh" ? city.nameZh : city.nameEn}
          </h2>
          <p className="text-sm text-muted">
            {cityDs.length} {lang === "zh" ? "个区" : "districts"}
          </p>
        </div>
        {mid && best && worst && (
          <div className="grid grid-cols-1 gap-2">
            <Stat
              label={lang === "zh" ? "本市中间值 · 模拟年收入" : "City midpoint · simulated annual income"}
        ...
        <p className="mt-auto text-sm leading-relaxed text-faint">{t.disclaimer}</p>
      </div>
    );
  }
  ...
  const allSorted = DISTRICTS.map((x) => ({
    d: x,
    m: modelDistrict(x, parentPct, gender),
  })).sort((a, b) => (outcome === "hhIncome" ? b.m.childRank - a.m.childRank : outcomeValue(b.m, outcome) - outcomeValue(a.m, outcome)));
    ...
    <div className="flex h-full flex-col overflow-y-auto">
      <div className="flex items-start justify-between gap-3 border-b border-line px-4 py-3">
        <div>
          ...
          <div className="text-sm text-muted">{lang === "zh" ? d.nameEn : d.nameZh}</div>
        </div>
        {onClose && (
      ...
      <div className="space-y-5 px-4 py-4">
        <div>
          <div className="text-sm tracking-wide text-muted uppercase">
            {t.outcomes[outcome]}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <Stat label={t.rank} value={`${Math.round(m.childRank)}%`} />
        </div>
        ...
        {worst && worst.d.adcode !== d.adcode && (
          <div>
            <div className="mb-1 text-sm tracking-wide text-muted uppercase">
              {t.exposure}
            </div>
            <p className="mb-2 text-sm leading-snug text-faint">{t.exposureHint}</p>
            <div className="grid grid-cols-2 gap-2 text-sm">
              <Stat
                label={t.moveBirth}
